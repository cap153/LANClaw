...
LANClaw 以 LANChat 网络中的对等节点身份运行，接收在线用户的消息和文件，转发给 Pi 进行 AI 处理，然后将回复发回给用户——全程无需修改 LANChat 一行代码。
...
2. **消息接收** — LANChat 用户通过 WebSocket/TCP 向 LANClaw 端口发送消息
3. **AI 处理** — LANClaw 调用 `pi -p --session <用户ID> "消息"` 处理
4. **回复** — Pi 的文本回复原样返回；Pi 生成的文件自动发送给用户
5. **文件处理** — 用户发送的文件保存后传给 Pi 分析（图片、文档等）
6. **定时任务** — Pi 通过 `lanclaw task` 命令行管理任务，LANClaw 后台调度执行
...
LANClaw 唯一直接解析的命令是 `/new`，用于重置你的对话 session。
